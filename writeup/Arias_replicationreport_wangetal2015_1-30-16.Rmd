...
http://web.stanford.edu/~jmarias/jmariasProject/experiment/mathmotivationsurveys.html
...
Power Analysis -- 
	*Note: from results presented in Table 9 (Wang et al., 2015), R-squared for finding of interest appears to be .16, but the delta r-squared is only .02*
	
Original effect size: total R-squared: .16 ; change in R-squared for variable of interest: .02

Power analysis: For effect size of .02, with 219 participants, 9 total predictors in model, and alpha of .05, estimated power achieved is:  54% 

Sample Size needed to achieve 80% power (assuming same effect size): 
Sample Size needed to achieve 90% power:
Sample Size needed to achieve 95% power:
...
### Math Motivation (3-item; see Chiu & Zeng, 2008) ######
  ...
  #No data prep needed aside from re-naming and calculating mean value for each participant
...
###MAKE THESE THE STANDARDIZED VALUES (z.genanx)
##CHANGE TO LM not GLM

mathperflm1 <- lm(pvt ~ age + sex + genanx + I(genanx^2), data = subjectmeans)

mathperflm2 <- lm(pvt ~ age + sex + genanx + I(genanx^2) + mathanx + mathmot, data = subjectmeans)

mathperflm3 <- lm(pvt ~ age + sex + genanx + I(genanx^2) + mathanx + mathmot + I(mathanx^2) 
                              + mathanx * mathmot, data = subjectmeans)

mathperflm4 <- lm(pvt ~ age + sex + genanx + I(genanx^2) + mathanx + mathmot + I(mathanx^2) 
...
mathperfglm4.alt <- glm(pvt ~ age + sex + genanx + I(genanx^2) + mathanx * mathmot + (I(mathanx^2) * mathmot), data = subjectmeans)
##Would this be the correct way to set up this model? 


##Look into using kable (in knitr package) to make nice table of model outputs 

kable(summary(modle1)$coef)
...
mathperflm2 <- lm(pvt ~ age + sex + genanx + I(genanx^2) + I(mathanx - 1) + mathmot, data = subjectmeans)

##Minus- 1 actually gives you 1 SD ABOVE b/c you areshifting the x axis "to the right" (mean becomes -1)


mathperflm4.alt <- lm(pvt ~ age + sex + genanx + I(genanx^2) + mathanx * I(mathmot - 1) + (I(mathanx^2) * I(mathmot - 1)), data = subjectmeans)
