...
#### Planned Sample:
To achieve 80% power, the planned sample will be **395** adult participants recruited from Amazon's Mechanical Turk platform. 
...
Please see below for quoted descriptions of the relevant elements of the analysis plan from the original study, which were also conducted by the current replication. The embedded figure below (Figure 3) shows a comparison of the model predictions for individuals at 1 SD above and 1 SD below in math motivation, and is from the original Wang et al. (2015). See further below for differences of note between original and replication in the analysis plan. 
...
#### Differences in Procedure/Analysis Plan:
...
### The data presented here is from Pilot B, conducted 3-14-16 with 3 MTurk participants
...
### Note that the predicted model plot above has only 1 line, but includes 2 linear models ("mathperflm4-above" and "mathperflm4_below")
The plot does not distinguish between these 2 models (nor looks like Figure 3 of original study), presumably because not enough data have been collected to generate differing models, but please check model construction and inform me if it needs to be corrected. 

#### Exploratory analyses

Will be conducting exploratory analyses on effect of education level, current student status, whether or not participant has school-aged children, and frequency with which they help children with math homework on math performance, both independent of and in conjunction with math anxiety and math motivation. See below for planned analyses and plots of data. 
...
#Calculate mean 
expsubmeans <- d %>% 
  group_by(gender, education, student, children, mathhwhelp, childAgeOld, age, workerid) %>% 
  summarise(mean.acc = mean(pvt.acc, na.rm=T), 
            mean.RT = mean(sec.RT, na.rm=T), 
            mean.mathmot = mean(mathmot, na.rm=T), 
            mean.mathanx = mean(mathanx, na.rm=T), 
...
```{r EXPLORATORY ANALYSES CORRELATIONS between variables of interest, eval=FALSE}
...
##Create table from dataframe: 
exploratory_cor_table <- corstarsl(expsubmeans) 

##To remove unwanted columns from dataframe before adding to correlation table (in order to replicate table from original study)

exploratory.cor.means <- expsubmeans %>% 
  select(age, gender, education, student, homelang, ethnicity, race, children, mathhwhelp, childAgeOld, mean.genanx, mean.mathanx, mean.mathmot, mean.acc, mean.RT, pvt)
##Create table from dataframe: 
concise_exploratory_cor_table <- corstarsl(exploratory.cor.means)


## exporting tables to html, if desired: 
#print.xtable(concise_exploratory_cor_table, type="html", file="concise_exploratory_cor_table.html")
...
######################################
###MODEL SPECIFIC TO PILOT B GROUP (no deviation in student or education status, so these were omitted)
exploreperflm.pilotB <- lm(z.pvt ~ age + gender + children + childAgeOld + mathhwhelp + z.mathanx, data = expsubmeans)
kable(summary(exploreperflm.pilotB)$coef)
######################################
...
######################################
###TOTAL MODEL SPECIFIC TO PILOT B GROUP (no deviation in student or education status, so these were omitted)
exploreperflm.TOTALpilotB <- lm(z.pvt ~ age + gender + children + childAgeOld + mathhwhelp + z.genanx + I(z.genanx^2) + z.mathanx * z.mathmot + (I(z.mathanx^2) * z.mathmot), data = expsubmeans)
kable(summary(exploreperflm.TOTALpilotB)$coef)
######################################
...
ggplot(expsubmeans, aes(x = z.mathanx, y = z.pvt)) + 
  geom_point() +
  geom_smooth(method = lm, data = exploreperflm.TOTALpilotB) +
  xlab("Math Anxiety") +
  ylab("PVT Efficiency Performance") +
...
#NOTE THAT THIS PLOT ONLY CONTAINS 3 TOTAL DATA POINTS FROM PILOT B
ggplot(expsubmeans, aes(x = z.mathanx, y = z.pvt)) + 
  geom_point() +
  geom_smooth(method = lm, data = exploreperflm.TOTALpilotB) +
  xlab("Math Anxiety") +
...
mathperflm4.TOTALpilotB_above <- lm(z.pvt ~ age + gender + children + childAgeOld + mathhwhelp + z.genanx + I(z.genanx^2) + z.mathanx * I(z.mathmot - 1) + (I(z.mathanx^2) * I(z.mathmot - 1)), data = expsubmeans)

kable(summary(mathperflm4.TOTALpilotB_above)$coef)



mathperflm4.TOTALpilotB_below <- lm(z.pvt ~ age + gender + children + childAgeOld + mathhwhelp + z.genanx + I(z.genanx^2) + z.mathanx * I(z.mathmot + 1) + (I(z.mathanx^2) * I(z.mathmot + 1)), data = expsubmeans)

kable(summary(mathperflm4.TOTALpilotB_below)$coef)
...
##ggplot of two predicted lines: for 
ggplot(expsubmeans, aes(x = z.mathanx, y = z.pvt)) + 
  geom_smooth(method = lm, data = mathperflm4.TOTALpilotB_above) +
  geom_smooth(method = lm, data = mathperflm4.TOTALpilotB_below) +
  xlab("Math Anxiety (Standardized)") +
  ylab("PVT Efficiency Performance (Standardized)") +
