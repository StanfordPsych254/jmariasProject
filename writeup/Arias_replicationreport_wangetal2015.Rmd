...
##### Planned Sample:
To achieve 80% power, the planned sample will be **395** adult participants recruited from Amazon's Mechanical Turk platform. Study 2 of Wang et al. (2015) used 237 undergraduate students, 56% female, mean age = 19.53 years old (*SD* = 2.01, range = 18 - 39), for their participant sample.
	

#### Materials
  
Please see below for quoted descriptions of the materials used in the original study, which were adapted for use by an online MTurk population in the present replication: 
...
MENTION ADDITIONAL DEMOGRAPHIC QUESTIONS TO BE OBTAINED
...
```{r EXPLORATORY ANALYSES Hierarchical Regression Models, eval=FALSE}
...
```{r EXPLORATORY ANALYSES plot of predicted values, eval=FALSE}

ggplot(expsubmeans, aes(x = z.mathanx, y = z.pvt)) + 
  geom_point() +
  geom_smooth(method = lm, data = exploreperflm4) +
  xlab("Math Anxiety") +
  ylab("PVT Efficiency Performance") +
  ggtitle("Math Performance on Math Anxiety") + 
    theme(plot.title = element_text(lineheight=.8, face="bold"))
